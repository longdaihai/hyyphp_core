...
class Model
{
    ...
    {
        //
        $confDB = Config::getAll('db');

        $options = array_merge($confDB, $options);
        ...
        if (isset($options[ 'DB_LOG' ]) && is_bool($options[ 'DB_LOG' ]))
        {
            $this->logging = $options[ 'DB_LOG' ];
        }
        ...
        {
            if (
                isset($options[ 'DB_PORT' ]) &&
                is_int($options[ 'DB_PORT' ] * 1)
            )
            {
            ...
            switch ($this->type)
            {
                ...
                case 'oracle':
                    $attr = [
                        'driver' => 'oci',
                        'dbname' => $options[ 'DB_HOST' ] ?
                            '//' . $options[ 'DB_HOST' ] . ($is_port ? ':' . $port : ':1521') . '/' . $options[ 'DB_NAME' ] :
                            $options[ 'DB_NAME' ]
                    ];

                    if (isset($options[ 'DB_CHARSET' ]))
                    {
                        $attr[ 'charset' ] = $options[ 'DB_CHARSET' ];
                    }
                ...
                case 'mssql':
                    if (isset($options[ 'driver' ]) && $options[ 'driver' ] === 'dblib')
                    {
                        $attr = [
                            'driver' => 'dblib',
                            'host' => $options[ 'DB_HOST' ] . ($is_port ? ':' . $port : ''),
                            'dbname' => $options[ 'DB_NAME' ]
                        ];
                    ...
                    else
                    {
                        $attr = [
                            'driver' => 'sqlsrv',
                            'Server' => $options[ 'DB_HOST' ] . ($is_port ? ',' . $port : ''),
        ...
        if (
            ...
            isset($options[ 'DB_CHARSET' ])
        )
        {
            $commands[] = "SET NAMES '{$options[ 'DB_CHARSET' ]}'" . (
        ...
        try {
            $this->pdo = new PDO(
                $dsn,
                isset($options[ 'DB_USER' ]) ? $options[ 'DB_USER' ] : null,
                isset($options[ 'DB_PWD' ]) ? $options[ 'DB_PWD' ] : null,
                $option
            );
            ...
            $time = number_format(t('HYYPHP_DB_EXCE', 1) * 1000, 5);
        }
        catch (PDOException $e) {
            throw new PDOException($e->getMessage());
        }
        Log::trace('SQL', "PDO连接 [{$time}s]");
    }
    ...
    public function exec($query, $map = [])
    {
        ...
        if ($statement)
        {
            ...
            // 计时
            $time = number_format(t('HYYPHP_DB_EXCE', 1), 5);
            Log::trace('SQL', $this->last() . " [{$time} s]");
    ...
    {
        return ':HyyPhp_' . $this->guid++ . '_HyyPhp';
    }
